 ...
 * install-restaged.ts
 *
 * For each {handle, url} in wave1-white-urls.json:
 ...
 *
 * Run: npx tsx scripts/install-restaged.ts
 */
...
const URLS_PATH = path.join(ROOT, "scripts/wave1-white-urls.json");
...
async function main() {
  ...
  console.log(`Installing ${handles.length} re-staged images...\n`);
  ...
  for (const handle of handles) {
    ...
    // Ensure output directory exists (it should already from catalog seeding)
    fs.mkdirSync(outDir, { recursive: true });

    // 1. Download to temp file
    const tmpPath = path.join(os.tmpdir(), `restage-${handle}-${Date.now()}.tmp`);
    try {
    ...
    const tmpOptPath = path.join(os.tmpdir(), `restage-${handle}-opt-${Date.now()}.webp`);
    try {
    ...
    if (fs.existsSync(outPath) && !fs.existsSync(origPath)) {
      ...
      process.stdout.write(`(backup exists, skipping). `);
    }
    ...
    const trivial = kb < 5; // < 5KB is suspiciously small
  ...
  if (errors.length > 0) {
    ...
    console.log("\nAll re-staged images installed successfully.");
  }
}
